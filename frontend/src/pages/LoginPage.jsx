...
export default function LoginPage() {
  ...
  useEffect(() => {
    const LOGIN_NOTICE_KEY = "quantstrike_login_notice";
    ...
    try {
      const messages = [];
      const reason = window.sessionStorage.getItem("quantstrike_logout_reason");
      if (reason) {
        messages.push(reason);
        window.sessionStorage.removeItem("quantstrike_logout_reason");
      }
  ...
  const handleSubmit = (event) => {
    ...
    axios
      ...
      .then((response) => {
        ...
        const isStaff = Boolean(response.data?.is_staff);
        setAuthUsername(usernameFromServer.toUpperCase(), {
          isSuperuser,
          isStaff,
        });
        setUserRole(isSuperuser);
