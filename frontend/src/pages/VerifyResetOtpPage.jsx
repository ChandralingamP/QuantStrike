...
export default function VerifyResetOtpPage() {
  ...
  useEffect(() => {
    ...
    try {
      ...
      const hint = location.state?.otpHint || parsed.otpHint;
      setInfoMessage(
        hint
          ? `We emailed a 6-digit OTP to ${parsed.email}.`
          : `We emailed a 6-digit OTP to ${parsed.email}.`
      );
    } catch (error_) {
  ...
  const handleSubmit = (event) => {
    ...
    axios
      .post(
        ...
        { email: pendingData.email, otp: trimmedOtp },
      ...
      .then((response) => {
        const payload = {
          ...
          otp: trimmedOtp,
          otpHint: response.data?.otp ?? pendingData.otpHint ?? null,
        };
      ...
      .catch((verifyError) => {
        const detail =
          ...
          verifyError.response?.data?.non_field_errors?.[0] ||
  ...
  const handleResendOtp = () => {
    ...
    axios
      ...
      .then((response) => {
        ...
        setInfoMessage(
          hint
            ? `We emailed a 6-digit OTP to ${pendingData.email}.`
            : `We emailed a 6-digit OTP to ${pendingData.email}.`
        );
      })
      .catch((error_) => {
    ...
    <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-950 to-slate-900 py-16">
      <div className="mx-auto flex w-full max-w-md flex-col gap-8 rounded-3xl border border-slate-800 bg-slate-900/70 p-8 text-white shadow-2xl shadow-black/40">
        <header className="space-y-2 text-center">
          ...
          <h1 className="text-3xl font-semibold">Verify reset OTP</h1>
